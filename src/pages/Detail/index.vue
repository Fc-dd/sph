<template>
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="mainCon">
        <!-- 左侧放大镜区域 -->
        <div class="previewWrap" ref="pre">
          <!--放大镜效果-->
          <Zoom
            :skuDefaultImg="skuInfo.skuDefaultImg"
            :preTopOrLeft="preTopOrLeft"
          />
          <!-- 小图列表 -->
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attr in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  ...
                  :key="val.id"
                  @click="changeAttr(val.id, attr.id)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a class="mins" @click="skuNum <= 1 ? (skuNum = 1) : skuNum--">
                  -
                </a>
              </div>
              <div class="add">
                <a @click="addCart(skuInfo.id, skuNum)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getItem() {
      this.$store.dispatch('detail/getItemList', this.$route.params.skuId)
    },
    changeAttr(sonId, fatherId) {
      // console.log(sonId,fatherId)
      this.$store.commit('detail/CHANGECHECKED', { sonId, fatherId })
    },
    changeSkuNum() {
      var regexp = new RegExp('^([1-9][0-9]*)$')
      if (regexp.test(this.skuNum)) {
        return
      } else {
        this.skuNum = 1
      }
    ...
    addCart(id,num) {
      // console.log(id,num)
      this.$store.dispatch('detail/addShopCart', { skuId: id, skuNum:num})
        .then(
          (value) => {
            sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
            sessionStorage.setItem('ATTR',JSON.stringify(this.spuSaleAttrList))
  ...
  mounted() {
    this.getItem()
    this.preTopOrLeft = {
      top: this.$refs.pre.offsetTop,
      left: this.$refs.pre.offsetLeft,
    }
  },
  computed: {
    ...mapState('detail', ['itemList']),
